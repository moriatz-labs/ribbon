    ...
    <Box as="section" id="providers" css={{ borderTop: "$subtle solid $border", background: "$muted", paddingBlock: "$10" }}>
      <Container>
        <Grid columns={{ initial: "1fr", lg: "minmax(0, .72fr) minmax(0, 1.28fr)" }} gap="$8" css={{ alignItems: "start" }}>
          <Stack gap="$3">
            <Text css={{ color: "$primary", fontFamily: "$ui", fontWeight: "$semibold" }}>Provider matrix</Text>
          ...
          <Box className="provider-table-wrap">
            <table className="provider-table">
              <caption>Ribbon capabilities and supported providers</caption>
              <thead>
                <tr>
              ...
              <tbody>
                {PROVIDER_CAPABILITIES.map((capability) => (
                  <tr key={capability.id}>
                    <td>
                      <Stack gap="$1">
                        <Text css={{ fontFamily: "$ui", fontWeight: "$semibold" }}>{capability.label}</Text>
                        <Text size="sm" css={{ color: "$mutedForeground" }}>{capability.role}</Text>
                      </Stack>
                    </td>
                    <td>
                      <Stack gap="$1">
                        <Text size="xs" css={{ color: "$primary", fontFamily: "$ui", fontWeight: "$semibold" }}>{capability.defaultProvider}</Text>
                        <Text size="xs" css={{ color: "$mutedForeground", fontFamily: "$ui" }}>{capability.alternativeProvider}</Text>
                      </Stack>
                    </td>
                  </tr>
                ))}
